          ...
          <div>
            ...
            <div className={style["metrics-row"]}>
              <div className={style["metrics-container"]}>
                ...
                <div className={style["metrics-value"]}>Average CPU Usage - {average_cpu ?? "N/A"}%</div>
              </div>
              <div className={style["metrics-container"]}>
                <div className={style["metrics-icon"]}>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                ...
                <div className={style["metrics-value"]}>Average Battery - {average_battery ?? "N/A"}%</div>
              </div>
              <div className={style["metrics-container"]}>
                <div className={style["metrics-icon"]}>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <rect x="3" y="3" width="18" height="18" rx="2" stroke="white" strokeWidth="2" />
                    <rect x="7" y="7" width="10" height="10" rx="1" fill="white" />
                    <path d="M12 7v6M9 10h6" stroke="#1F2937" strokeWidth="1.5" />
                  </svg>
                </div>
                <div className={style["metrics-value"]}>Average Memory - {average_memory ?? "N/A"}%</div>
              </div>
              <div className={style["metrics-container"]}>
                <div className={style["metrics-icon"]}>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <circle cx="12" cy="12" r="10" stroke="white" strokeWidth="2" />
                    <text x="12" y="16" textAnchor="middle" fontSize="10" fill="#1F2937">
                      HS
                    </text>
                  </svg>
                </div>
                <div className={style["metrics-value"]}>Health Score - {health_score ?? "N/A"}</div>
              </div>
            </div>
              ...
              <div className={style["additional-details-table"]}>
                {Object.entries(rest).map(([key, value]) => (
                  <div className={style["additional-details-row"]} key={key}>
                    <div className={style["additional-details-label"]}>{key.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}</div>
                ...
                {uptime &&
                  Object.entries(uptime).map(([key, value]) => (
                    <div className={style["additional-details-row"]} key={key}>
                      <div className={style["additional-details-label"]}>{key.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}</div>
                      <div className={style["additional-details-value"]}>{value?.toString()}</div>
                    </div>
                  ))}
              </div>
            </div>
            ...
            {metrics_history.length === 0 ? (
              ...
              <div>
                {metrics_history.map((metric, idx) => (
                  <div
                    key={idx}
                    className={style["additional-details-container"]}
                    style={{ marginBottom: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", borderRadius: 8, background: "#fff", padding: 16 }}
                  >
                    <div className={style["additional-details-table"]}>
